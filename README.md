...
- 코드그룹에 맞는 코드인지 확인하기 위해 코드 테이블 조회
- 환자, 환자방문 저장 및 수정 시 임의의 값을 넣는 것을 방지하기 위해 코드 확인 로직 추가
- Enum 으로 관리하게 되면 코드그룹 테이블에 데이터가 추가될 때마다 Enum 에도 값을 추가하여 배포해야되기 때문에 이를 방지하기 위해 코드그룹 테이블 생성 및 관리


**추가 된 필드**

- 환자 테이블의 삭제 필드는 추후에 언제 데이터가 필요하게 될지 몰라 soft delete 처리를 해줬습니다.
    - 병원 데이터를 삭제할 때 DB DELETE 쿼리문은 추후 병원 데이터에대해 추적하기 힘들기 때문에 삭제여부 필드를 추가하여 병원 삭제를 관리합니다.
...
- 환자가 1명도 없을 경우에는 Exception이 반환됩니다.
...
- 환자방문이 존재하지 않을 경우에는 null을 반환하고, 존재할 경우에는 가장 최근의 환자방문 이력을 반환합니다.
...
- 병원과 환자가 변경될 일은 없기 때문에 접수일시와 방문상태코드만 변경할 수 있습니다.
...
- 코드테이블에서 방문상태코드 중 `3`은 `취소`를 의미하기 때문에 삭제 할 때 방문상태코드를 3으로 변경해줍니다.
